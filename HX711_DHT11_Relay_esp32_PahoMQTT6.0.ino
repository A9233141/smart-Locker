...
const char *ssid = "camel";
const char *password = "aa01200120";
...
float humLimit = 110;    // 濕度警示上限值
float tempLimit = 110;  // 溫度警示上限值
int weight = 100;     //重量警示上限值

unsigned long currentTime;
int count = 0;
int sendTime = 60;      // DHT11量測結果發送時間以秒為單位
int checkTime = 30;      // DHT11量測結果檢查時間以秒為單位
...
void setup() {
  ...
  //HX711磅秤的啟動
  Serial.println("已發布溫度YourEsp32/esp32s_temp");
  Serial.println("已發布濕度YourEsp32/esp32s_humd");
  Serial.println("已發布重量YourEsp32/esp32s_HHX711");
  Serial.println("磅秤以初始化完畢，可以開始使用");//初始化稱重傳感器
...
void loop() {
  ...
  if (now - lastMsg > updateCycle) //用現在的時間上減掉上一個訊息所發送的時間看，看有沒有大於updateCycle，如果沒有大5秒的話就不會進入這一個迴圈。
  {
    ...
    if (isnan(humidity) || isnan(temperature)) { //確定是否有讀到溫度和濕度
      ...
      Serial.println("向MQTTgo.io發布訊息: ");//發布訊息
